# Prompt for Claude Code Instance 1: Compression Efficiency Test

## Request
I need you to run a 12-hour compression efficiency test for the META-OPT-QUANT V6 system. This test should focus on achieving maximum compression ratios using the autonomous test framework.
...
2. Create the test implementation file `run_compression_test.py`:
...
3. Launch the test:
```bash
python3 run_compression_test.py > compression_test_output.log 2>&1 &
```

## Expected Results
- Duration: 12 hours
- Target compression ratio: 30x (stretch goal: 48x)
- Focus on high-symmetry datasets
- Checkpoints every 60 minutes
- Final report: `compression_efficiency_test_report.json`
...
Please implement and run this compression efficiency test. The test will run autonomously for 12 hours and generate comprehensive compression performance metrics.